...
//PubSubClient mqttClient(bear);
#endif //ENABLE_MQTT


char g_szRecvBuff[5000];

ESP8266WebServer server(80);
SimpleTimer timer;
circular_log<500> g_log;
bool ntpTimeReceived = false;
int g_baudRate = 0;


class PylontechMonitoring{
  ...
  void setup() {  
    ...
    ArduinoOTA.begin();
    server.on("/", std::bind(&PylontechMonitoring::handleRoot, this));
    server.on("/log", std::bind(&PylontechMonitoring::handleLog, this));
    server.on("/req", std::bind(&PylontechMonitoring::handleReq, this));
    server.on("/jsonOut", std::bind(&PylontechMonitoring::handleJsonOut, this));
    server.on("/reboot", [](){
      ESP.restart();
    });
  ...
  void handleReq()
  {
    ...
    {
      if(server.hasArg("code")) server.send(500, "text/plain", server.arg("code")+"?");
  ...
  void syncTime()
  {
    ...
    {
      timer.setTimeout(5000, (timer_callback) &PylontechMonitoring::syncTime); //try again in 5 seconds
    }
  }
  */
  void wakeUpConsole()
  ...
  */
  ...
  {
    mqttConnection->mqtt_publish_f("soc",          g_stack.soc,                lastSentData.soc,                0, forceUpdate);
    mqttConnection->mqtt_publish_f("temp",         (float)g_stack.temp/1000.0, (float)lastSentData.temp/1000.0, 0, forceUpdate);
    mqttConnection->mqtt_publish_i("estPowerAC",   g_stack.getEstPowerAc(),    lastSentData.getEstPowerAc(),   10, forceUpdate);
    mqttConnection->mqtt_publish_i("battery_count",g_stack.batteryCount,       lastSentData.batteryCount,       0, forceUpdate);
    mqttConnection->mqtt_publish_s("base_state",   g_stack.baseState,          lastSentData.baseState            , forceUpdate);
    mqttConnection->mqtt_publish_i("is_normal",    g_stack.isNormal() ? 1:0,   lastSentData.isNormal() ? 1:0,   0, forceUpdate);
  }
 ...
 */ 
    //next: read data from battery and send via MQTT (but only once per MAX_PUSH_FREQ_SEC seconds)
    static unsigned long g_lastDataSent = 0;
    bool tts = (millis()/1000 - g_lastDataSent) > MAX_PUSH_FREQ_SEC;
    //Serial.print("  tts: "); Serial.print(tts); 
    //Serial.print(" res: "); Serial.print(rsr);  
    if(tts && sendCommandAndReadSerialResponse("pwr") == true){
      ...
      callCnt++;
      //Serial.print(callCnt);
      //Serial.print(" "); 
      g_lastDataSent = millis()/1000;
